...
const getBaseProducts = async (request, response) => {
  await pool.query(
    "SELECT * FROM baseproducts ORDER BY product_id ASC",
    (error, results) => {
      if (error) {
...
const getBaseProductByID = async (request, response) => {
  ...
  await pool.query(
    "SELECT * FROM baseproducts WHERE product_id = $1",
    [id],
    (error, results) => {
      if (error) {
        response.status(500).send(error);
...
const getBaseProductsByCategoryID = async (request, response) => {
  ...
  await pool.query(
    "SELECT * FROM baseproducts WHERE category_id = $1",
    [id],
    (error, results) => {
      if (error) {
        response.status(500).send(error);
...
const getBaseProductsByColorID = async (request, response) => {
  ...
  await pool.query(
    "SELECT * FROM baseproducts WHERE color_id = $1",
    [id],
    (error, results) => {
      if (error) {
        response.status(500).send(error);
...
const createBaseProduct = async (request, response) => {
  ...
  await pool.query(
    "INSERT INTO baseproducts (name, category_id, price, color_id, images) VALUES ($1, $2, $3, $4, $5)  RETURNING *",
    [name, category_id, price, color_id, images],
    (error, results) => {
      if (error) {
        throw error;
      }
      ...
      response
        .status(201)
        .send(
          `BaseProduct added with ID: ${results.rows[0].id}, named ${results.rows[0].name}`
        );
    }
  );
};
...
const deleteProductByID = async (request, response) => {
  ...
  await pool.query(
    "DELETE FROM baseproducts WHERE product_id = $1",
    [id],
    (error, results) => {
      if (error) {
        response.status(500).send(error);
