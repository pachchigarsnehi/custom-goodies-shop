...
const getCartAll = async (request, response) => {
    await pool.query('SELECT * FROM cart ORDER BY cart_id ASC', (error, results) => {
        if (error) {
...
const getCartByCustomerID = async (request, response) => {
    ...
    await pool.query('SELECT * FROM cart WHERE customer_id = $1', [customer_id], (error, results) => {
      if (error) {
        response.status(500).send(error);
      }
      response.status(200).json(results.rows)
...
const createCartEntry = async (request, response) => {
    const { customer_id, product_id, font, quote, price, quantity } = request.body
    console.log(request.body)
    await pool.query('INSERT INTO cart (customer_id, product_id, font, quote, price, quantity) VALUES ($1, $2, $3, $4, $5, $6)  RETURNING *', [customer_id, product_id, font, quote, price, quantity], (error, results) => {
      if (error) {
        throw error
      }
      console.log(results.rows[0].cart_id)
      response.status(201).send(`cart entry added with ID: ${results.rows[0].cart_id}`)
    })
  }
  ...
  const deleteCartEntry = async (request, response) => {
    ...
    await pool.query('DELETE FROM cart WHERE cart_id = $1', [cart_id], (error, results) => {
      if (error) {
        response.status(500).send(error);
      }
